$(document).ready(function(){
    ...
    let main_arrow = document.querySelector('.ask_des_arrow'), bool;
    let html, name = 'name';
    ...
    function isEmployees(){
        ...
        $('.ask_des_arrow').on('click', function(){ 
            if(!bool){
                ascendingSorting(obj);
                html = 0;
                
            }else{
                 descendingSorting(obj);
                 html = 0;
            }
            $('#table').text(html);
        })
    ...
    function ascendingSorting(obj){
        console.log(obj.sort((a, b) => (a[name] || "").toString().localeCompare((b[name] || '').toString())));
        main_arrow.classList.add('rotate');
        // html_sort = $(".item_table").append(asc); 
        bool = true; console.log('asc');
    }    
    function descendingSorting(obj){
        console.log(obj.sort((a, b) => (b[name] || "").toString().localeCompare((a[name] || '').toString())));
        main_arrow.classList.remove('rotate');
        // html_sort = $(".item_table").html(des);  
        bool = false; console.log('des');
    }
